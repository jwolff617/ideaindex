...
const IdeaCard = ({ idea }) => {
  ...
  const generateTitleFromBody = async () => {
    if (!replyBody.trim()) {
      toast.error('Write your idea first, then AI will generate a title');
      return;
    }
    ...
    try {
      ...
      setNewIdeaTitle(response.data.title);
      toast.success('✨ Title generated by AI!');
    } catch (error) {
      console.error('Failed to generate title', error);
      // Fallback: use first sentence
      const fallback = replyBody.split('.')[0].slice(0, 50);
      setNewIdeaTitle(fallback);
      toast.info('Used first sentence as title');
    } finally {
      setGeneratingTitle(false);
    }
  };

  const handlePostAsNewIdeaChange = async (checked) => {
    setPostAsNewIdea(checked);
    
    // Auto-generate title when checkbox is checked
    if (checked && replyBody.trim() && !newIdeaTitle) {
      await generateTitleFromBody();
    }
  };
